...
void RetireRikishi(std::vector<Rikishi>& rikishiVector, std::vector<Player>& players) {
	std::vector<std::string> retiredNames;

	// Identify rikishi to retire
	...
	rikishiVector.erase(std::remove_if(rikishiVector.begin(), rikishiVector.end(),
		[&](const Rikishi& r) {
			bool shouldRetire = (r.rank == "Yokozuna" && r.wins < r.losses) ||
								(r.rank == "Ozeki" && r.wins < r.losses && r.age > 31);  // ||
...
//								(r.rank.find("Maegashira") != std::string::npos && r.age >= 31 && r.spirit == 0);
	...
	// Remove retired rikishi from players' rikishiList
	for (auto& player : players) {
		player.rikishiList.erase(std::remove_if(player.rikishiList.begin(), player.rikishiList.end(),
			[&](const Rikishi& r) {
				return std::find(toRetire.begin(), toRetire.end(), r.name) != toRetire.end();
			}), player.rikishiList.end());
	}
...
void RetireWave2(std::vector<Rikishi>& rikishiVector, std::vector<Player>& players,
                 const std::vector<std::pair<std::string, std::string>>& previousRanks) {
    std::vector<std::string> toRetire;
    ...
    for (auto& rikishi : rikishiVector) {
        if (rikishi.rank == "Juryo 1" || rikishi.rank == "Juryo 2") {
            // Find previous rank of this rikishi
            auto it = std::find_if(previousRanks.begin(), previousRanks.end(),
                [&](const std::pair<std::string, std::string>& p) { return p.first == rikishi.name; });
    ...
    // Remove retired rikishi from rikishiVector
    rikishiVector.erase(std::remove_if(rikishiVector.begin(), rikishiVector.end(),
        [&](const Rikishi& r) {
            return std::find(toRetire.begin(), toRetire.end(), r.name) != toRetire.end();
        }), rikishiVector.end());
    ...
    // Log retirements
    if (!toRetire.empty()) {
        AnsiString logMessage = "Wave 2 Retirements: ";
        for (const auto& name : toRetire) {
            logMessage += AnsiString(name.c_str()) + ", ";
        }
        logMessage = logMessage.SubString(1, logMessage.Length() - 2);
        // RyogokuForm->MemoLog->Lines->Add(logMessage);
        ...
        // RyogokuForm->MemoLog->Lines->Add("No retirements in Wave 2.");
        // RyogokuForm->MemoLog->Text += "No retirements in Wave 2.\n";
    }
}

void RetireWave3(std::vector<Rikishi>& rikishiVector, std::vector<Player>& players) {
    std::vector<std::string> toRetire;

    for (auto& rikishi : rikishiVector) {
        // Check if the rikishi is between Maegashira 1 and Juryo 2
        if ((rikishi.rank.find("Maegashira") != std::string::npos || rikishi.rank.find("Juryo") != std::string::npos)) {
            int spiritLoss = 0;

            // Determine spirit loss based on age
            if (rikishi.age >= 31 && rikishi.age <= 33) {
                spiritLoss = 1;
    ...
    // Log retirements
    if (!toRetire.empty()) {
        AnsiString logMessage = "Wave 3 Retirements: ";
        for (const auto& name : toRetire) {
            logMessage += AnsiString(name.c_str()) + ", ";
        }
        logMessage = logMessage.SubString(1, logMessage.Length() - 2);
        ...
        // RyogokuForm->MemoLog->Lines->Add("No retirements in Wave 3.");
        // RyogokuForm->MemoLog->Text += "No retirements in Wave 3.\n";
    }
}

std::vector<std::pair<std::string, std::string>> GetRikishiRankSnapshot(const std::vector<Rikishi>& rikishiVector) {
    std::vector<std::pair<std::string, std::string>> snapshot;
    for (const auto& rikishi : rikishiVector) {
        snapshot.emplace_back(rikishi.name, rikishi.rank);
    }
    return snapshot;
}
...
//	RyogokuForm->MemoLog->Text += "Retirements have gone through ...\n";
	...
	auto previousRanks = GetRikishiRankSnapshot(rikishiVector);
